...
fn main() {
    ...
    let cuda_path = Some(Path::new("C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v12.6").to_path_buf());
    let sycl_path = Some(Path::new("C:/Program Files (x86)/Intel/oneAPI/compiler/2025.0/include").to_path_buf());
    let opencl_path = Some(Path::new("C:/Program Files/NVIDIA GPU Computing Toolkit/CUDA/v12.6/lib/x64").to_path_buf());
    ...
    if let Some(ref path) = sycl_path {
        ...
        println!("cargo:warning=Found SYCL installation at: {}", path.display());
    ...
    if let Some(ref path) = cuda_path {
        ...
        println!("cargo:warning=Found CUDA installation at: {}", path.display());
        println!("cargo:rustc-cfg=feature=\"cuda\"");
    }
    
    if let Some(ref path) = opencl_path {
        println!("cargo:rustc-env=OPENCL_PATH={}", path.display());
        println!("cargo:warning=Found OpenCL installation at: {}", path.display());
...
fn is_openmp_supported() -> bool {
    ...
    let status = if cfg!(target_os = "windows") {
        Command::new("cl")
                .args(["/openmp", format!("/I{}", intel_include_path).as_str(), src_path.to_str().unwrap(), "/Fe:", out_path.to_str().unwrap()])
                .status()
    } else {
        Command::new("g++")
                .args(["-fopenmp", "-o", out_path.to_str().unwrap(), src_path.to_str().unwrap()])
                .status()
    };
