...
impl PolarCode {
    ...
    pub fn with_params(
        ...
        if !block_length.is_power_of_two() {
            return Err(Error::InvalidInput(
                "Block length must be a power of 2".into(),
            ));
        }

        if message_length >= block_length {
            return Err(Error::InvalidInput(
                "Message length must be less than block length".into(),
            ));
        }
    ...
    fn sc_decode_recursive(
        ...
        for i in 0..half_length {
            let u_i = if decoded_bits[start + i] == 1 {
                -1.0
            } else {
                1.0
            };
