...
impl Ldpc {
    ...
    pub fn new(hardware_accelerator: Arc<dyn HardwareAccelerator>) -> Result<Self> {
        ...
        let parity_check_matrix =
            Self::generate_parity_check_matrix(codeword_length, message_length)?;

        // Generate generator matrix from parity check matrix
        let generator_matrix =
            Self::generate_generator_matrix(&parity_check_matrix, message_length)?;
    ...
    fn belief_propagation_decode(&self, llr: &[f64]) -> Result<Vec<u8>> {
        ...
        let mut check_to_var =
            vec![vec![0.0; self.parity_check_matrix.col_to_rows[0].len()]; self.codeword_length];
        let mut var_to_check = vec![
            vec![0.0; self.parity_check_matrix.row_to_cols[0].len()];
            self.parity_check_matrix.rows
        ];
        ...
        for j in 0..self.codeword_length {
            for (_idx, &i) in self.parity_check_matrix.col_to_rows[j].iter().enumerate() {
                var_to_check[i][self.parity_check_matrix.row_to_cols[i]
                    .iter()
                    .position(|&x| x == j)
                    .unwrap()] = llr[j];
            }
        ...
        for _ in 0..self.max_iterations {
            ...
            for i in 0..self.parity_check_matrix.rows {
                for (_j_idx, &j) in self.parity_check_matrix.row_to_cols[i].iter().enumerate() {
                    ...
                    check_to_var[j][self.parity_check_matrix.col_to_rows[j]
                        .iter()
                        .position(|&x| x == i)
                        .unwrap()] = sign * magnitude.exp();
                }
            }
            ...
            let hard_decision: Vec<u8> = variable_nodes
                .iter()
    ...
    fn received_to_llr(&self, received: &[f64], snr_db: f64) -> Vec<f64> {
        received.iter().map(|&r| 2.0 * r / snr_db).collect()
    }
}
