...
impl ConvolutionalCode {
    ...
    pub fn with_params(
        ...
        if code_rate_numerator >= code_rate_denominator {
            return Err(Error::InvalidInput(
                "Code rate must be less than 1".into(),
            ));
        }
    ...
    fn encode_message(&self, message: &[u8]) -> Result<Vec<u8>> {
        ...
        // Encode bits
        let mut encoded_bits = Vec::with_capacity(message_bits.len() * self.code_rate_denominator / self.code_rate_numerator);
    ...
    fn decode_codeword(&self, codeword: &[u8]) -> Result<Vec<u8>> {
        ...
        for t in (0..symbols.len()).rev() {
            ...
            let input = if trellis.next_states[prev_state][1] == state { 1 } else { 0 };
    ...
    pub fn puncture(&self, codeword: &[u8], pattern: &[u8]) -> Result<Vec<u8>> {
        if pattern.is_empty() {
            return Err(Error::InvalidInput("Puncturing pattern cannot be empty".into()));
        }
    ...
    pub fn depuncture(&self, punctured: &[u8], pattern: &[u8], erasure_value: u8) -> Result<Vec<u8>> {
        if pattern.is_empty() {
            return Err(Error::InvalidInput("Puncturing pattern cannot be empty".into()));
        }
...
impl ErrorCorrectionAlgorithm for ConvolutionalCode {
    ...
    fn encode(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if self.hardware_accelerator.is_available() && self.hardware_accelerator.supports_convolutional() {
    ...
    fn generate_lookup_tables(&self, path: &Path) -> Result<()> {
        ...
        if let Some(trellis) = &self.trellis {
            ...
            let next_states_data = bincode::serialize(&trellis.next_states)
                .map_err(|_| Error::BinarySerialization(bincode::Error::new(bincode::ErrorKind::Custom("Failed to serialize next states".to_string()))))?;
            
            std::fs::write(next_states_path, next_states_data)?;
            
            // Save outputs
            let outputs_path = conv_path.join("outputs.bin");
            let outputs_data = bincode::serialize(&trellis.outputs)
                .map_err(|_| Error::BinarySerialization(bincode::Error::new(bincode::ErrorKind::Custom("Failed to serialize outputs".to_string()))))?;
        ...
        let poly_data = bincode::serialize(&self.generator_polynomials)
            .map_err(|_| Error::BinarySerialization(bincode::Error::new(bincode::ErrorKind::Custom("Failed to serialize generator polynomials".to_string()))))?;
    ...
    fn supports_hardware_acceleration(&self) -> bool {
        self.hardware_accelerator.is_available() && self.hardware_accelerator.supports_convolutional()
    }
...
impl fmt::Debug for ConvolutionalCode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("ConvolutionalCode")
            ...
            .field("code_rate", &format!("{}/{}", self.code_rate_numerator, self.code_rate_denominator))
