...
impl IGPUAccelerator {
    ...
    pub fn new(galois_field: Arc<GaloisField>) -> Result<Self> {
        ...
        if platforms.is_empty() {
            return Err(Error::HardwareUnavailable("No OpenCL platforms found".into()));
        }
        ...
        if device_id.is_none() {
            return Err(Error::HardwareUnavailable("No OpenCL GPU device found".into()));
        }
    ...
    pub fn new(galois_field: Arc<GaloisField>) -> Result<Self> {
        Err(Error::HardwareUnavailable("OpenCL is not available in this build".into()))
    }
    ...
    fn initialize_device_tables(&mut self) -> Result<()> {
        ...
        let context = self.context.downcast_ref::<opencl3::context::Context>()
    ...
    fn matrix_multiply_opencl(
        ...
        Err(Error::UnsupportedOperation("OpenCL matrix multiplication is currently disabled due to API incompatibility".into()))
    }
    ...
    fn element_wise_opencl(
        ...
        // OpenCL implementation is temporarily disabled due to API incompatibility
        Err(Error::UnsupportedOperation("OpenCL element-wise operation is currently disabled due to API incompatibility".into()))
    }
}
...
impl HardwareAccelerator for IGPUAccelerator {
    ...
    fn perform_tensor_operation(&self, op: TensorOperation) -> Result<()> {
        match op {
            ...
            TensorOperation::Convolution { .. } => {
                Err(Error::UnsupportedOperation("Convolution not yet implemented for OpenCL".into()))
            }
        }
    }
    ...
    fn calculate_syndromes(&self, data: &[u8], syndrome_count: usize) -> Result<Vec<u16>> {
        ...
        #[cfg(not(feature = "opencl"))]
        {
            Err(Error::UnsupportedOperation("OpenCL is not available for syndrome calculation".into()))
        }
    }
    ...
    fn multiply_vec(&self, a: &[u16], b: &[u16]) -> Result<Vec<u16>> {
        ...
        #[cfg(not(feature = "opencl"))]
        {
            Err(Error::UnsupportedOperation("OpenCL is not available for vector multiplication".into()))
        }
    }
    ...
    fn add_vec(&self, a: &[u16], b: &[u16]) -> Result<Vec<u16>> {
        ...
        #[cfg(not(feature = "opencl"))]
        {
            Err(Error::UnsupportedOperation("OpenCL is not available for vector addition".into()))
        }
    }
    ...
    fn polynomial_eval_batch(&self, polys: &[Vec<u16>], points: &[u16]) -> Result<Vec<Vec<u16>>> {
        ...
        #[cfg(not(feature = "opencl"))]
        {
            Err(Error::UnsupportedOperation("OpenCL is not available for polynomial evaluation".into()))
        }
    }
}
...
impl IGPUAccelerator {
    ...
    fn calculate_syndromes_impl(&self, _data: &[u8], _syndrome_count: usize) -> Result<Vec<u16>> {
        // OpenCL implementation is temporarily disabled due to API incompatibility
        Err(Error::UnsupportedOperation("OpenCL syndrome calculation is currently disabled".into()))
    }
    ...
    fn multiply_vec_impl(&self, _a: &[u16], _b: &[u16]) -> Result<Vec<u16>> {
        // OpenCL implementation is temporarily disabled due to API incompatibility
        Err(Error::UnsupportedOperation("OpenCL vector multiplication is currently disabled".into()))
    }
    ...
    fn add_vec_impl(&self, _a: &[u16], _b: &[u16]) -> Result<Vec<u16>> {
        // OpenCL implementation is temporarily disabled due to API incompatibility
        Err(Error::UnsupportedOperation("OpenCL vector addition is currently disabled".into()))
    }

    /// Evaluate polynomials in batch using OpenCL (implementation).
    #[cfg(feature = "opencl")]
    fn polynomial_eval_batch_impl(&self, _polys: &[Vec<u16>], _points: &[u16]) -> Result<Vec<Vec<u16>>> {
        // OpenCL implementation is temporarily disabled due to API incompatibility
        Err(Error::UnsupportedOperation("OpenCL polynomial evaluation is currently disabled".into()))
    }
}
...
impl HardwareAccelerator for SYCLAccelerator {
    ...
    fn calculate_syndromes(&self, _data: &[u8], _syndrome_count: usize) -> Result<Vec<u16>> {
        Err(Error::UnsupportedOperation("SYCL implementation not available".into()))
    }
...
pub fn register_opencl_accelerators(accelerators: &mut Vec<Arc<dyn HardwareAccelerator>>) -> Result<()> {
