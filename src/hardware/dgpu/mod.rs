...
impl GPUAccelerator {
    ...
    pub fn new(galois_field: Arc<GaloisField>) -> Result<Self> {
        ...
        let major = device.get_attribute(rustacuda::device::DeviceAttribute::ComputeCapabilityMajor)?;
                        ...
                        for ic in 0..input_channels {
                            for kh in 0..kernel_height {
                                for kw in 0..kernel_width {
                                    ...
                                    if ih >= padding_h && ih < input_height + padding_h && 
                                       iw >= padding_w && iw < input_width + padding_w {
                                        let input_idx = b * (input_height * input_width * input_channels) +
                                                       (ih - padding_h) * (input_width * input_channels) +
                                                       (iw - padding_w) * input_channels + ic;
                                        
                                        let kernel_idx = kh * (kernel_width * input_channels * output_channels) +
...
impl HardwareAccelerator for GPUAccelerator {
    ...
    fn calculate_syndromes(&self, data: &[u8], syndrome_count: usize) -> Result<Vec<u16>> {
        ...
        for i in 0..syndrome_count {
            ...
            for j in 0..data.len() {
                ...
                alpha_power = self.galois_field.multiply(alpha_power, (alpha_i + 1) as u16);
            }
    ...
    fn multiply_vec(&self, a: &[u16], b: &[u16]) -> Result<Vec<u16>> {
        // TEMPORARY STUB: CPU-based implementation until CUDA macro issues are fixed
        if a.len() != b.len() {
            return Err(Error::InvalidInput("Input vectors must have the same length".into()));
        }
...
impl CuDNNAccelerator {
    ...
    pub fn new(galois_field: Arc<GaloisField>, cuda_accelerator: Arc<GPUAccelerator>) -> Result<Self> {
        Ok(Self {
            galois_field,
            cuda_accelerator,
            stats: RwLock::new(HardwareStatistics::default()),
...
impl HardwareAccelerator for CuDNNAccelerator {
    ...
    fn calculate_syndromes(&self, data: &[u8], syndrome_count: usize) -> Result<Vec<u16>> {
        ...
        self.cuda_accelerator.calculate_syndromes(data, syndrome_count)
    }
...
pub fn register_cuda_accelerators(accelerators: &mut Vec<Arc<dyn HardwareAccelerator>>) -> Result<()> {
    ...
    if GPUAccelerator::is_available() {
        ...
        if CuDNNAccelerator::is_available() {
            ...
            let cudnn_accelerator = Arc::new(CuDNNAccelerator::new(galois_field, cuda_accelerator)?);
