...
impl GaloisField {
    ...
    pub fn new(field_polynomial: u32) -> Self {
        ...
        let tables = GaloisTables {
            ...
            exp_table: field
                .exp_table
                .read()
                .expect("Failed to read exp_table")
                .clone(),
            log_table: field
                .log_table
                .read()
                .expect("Failed to read log_table")
                .clone(),
        };
    ...
    fn generate_tables(&self) {
        let mut exp_table = self
            .exp_table
            .write()
            .expect("Failed to write to exp_table");
        let mut log_table = self
            .log_table
            .write()
            .expect("Failed to write to log_table");
        ...
        *self
            .tables_normalized
            .write()
            .expect("Failed to write to tables_normalized") = true;
    }
    ...
    fn normalize_tables(&self) {
        if *self
            .tables_normalized
            .read()
            .expect("Failed to read tables_normalized")
        {
            return;
        }
    ...
    pub fn generate_lookup_tables(&self) -> Result<()> {
        ...
        bincode::serialize_into(file, &tables).map_err(|e| Error::BinarySerialization(e))?;
    ...
    pub fn log(&self, value: u16) -> u16 {
        assert!(
            value != 0,
            "Cannot compute logarithm of zero in Galois Field"
        );
        assert!(
            value < self.field_size as u16,
    ...
    pub fn inverse(&self, value: u16) -> u16 {
        assert!(
            value != 0,
            "Zero has no multiplicative inverse in Galois Field"
        );
